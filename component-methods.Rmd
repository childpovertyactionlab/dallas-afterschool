...
```{r, dot density population, echo = FALSE, message=FALSE, warning=FALSE, layout="l-body", fig.height=8}
ggplot() +
...
programcount <- full_join(das, cacfp) %>%
  full_join(hhs) %>%
  select(source, count, capacity, avgseats)

programcount %>%
  knitr::kable(.,
               col.names = c('', 'Total Programs', 'Seat Capacity', 'Average Seats'))
...
# Generate Plot
ggplot() +
  geom_sf(data = cacfp_programs, color = "orange") +
  geom_sf(data = hhs_programs, color = "red") +
  geom_sf(data = das_programs, color = "blue") +
  geom_sf(data = dallasCounty, color = "#323232", fill = NA, size = 2) +
  geom_sf(data = highways, color = "#323232", size = 1) +
#  scale_fill_brewer(name = "",
#                    palette = "BuPu",
...
#                                         nrow = 1,
#                                         label.position = "top")) +
  labs(title = "Afterschool Programs in Dallas County",
       subtitle = "DAS, CACFP, and HHS",
       x = "",
       y = "",
       color = "") +
  theme_void() +
