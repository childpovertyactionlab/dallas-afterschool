...
DallasStreets <- st_read(paste0(datalib, "Living Wage Jobs/04_Projects/Housing Audit/Data/Housing Audit Current State.gpkg"), layer = "City of Dallas Streets") %>%
  filter(CLASS %in% c("DALLAS AREA HIGHWAY", "HIGHWAY", "MAJOR ARTERIAL"))

DallasHighway <- DallasStreets %>%
  filter(CLASS %in% c("DALLAS AREA HIGHWAY", "HIGHWAY")) %>%
  st_transform(crs = 6584)

DallasMajor <- DallasStreets %>%
  filter(CLASS %in% "MAJOR ARTERIAL") %>%
  st_transform(crs = 6584)
...
```{r, map of DAS afterschool programs in county, warning=FALSE}
...
```{r, dot density population, warning=FALSE}
ggplot() +
